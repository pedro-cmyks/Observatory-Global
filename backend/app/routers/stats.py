        ...
        async with db.pool.acquire(timeout=5) as conn:
            ...
            try:
                nlp_row = await conn.fetchrow(
                    """
                    SELECT
                        COALESCE(SUM(unprocessed_24h), 0) AS unprocessed_24h,
                        COALESCE(SUM(unprocessed_total), 0) AS unprocessed_total,
                        MIN(oldest_unprocessed_at) AS oldest_unprocessed_at,
                        MIN(lag_minutes) AS lag_minutes,
                        MAX(last_run_at) AS last_run_at,
                        AVG(last_run_duration_seconds) AS avg_duration
                    FROM nlp_progress
                    """,
                    timeout=3.0,
                )
                if nlp_row is not None:
                    ...
                    nlp_block = {
                        ...
                        "last_run_duration_seconds": (
                            round(float(nlp_row["avg_duration"]), 1)
                            if nlp_row["avg_duration"] is not None else None
                        ),
                    }
            except Exception:
                nlp_block = None
