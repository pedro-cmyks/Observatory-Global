...
## Regression & Recovery (2025-11-24)

### What Broke
- **Issue**: Attempted to replace `ScatterplotLayer` with `HeatmapLayer` and add `wrapLongitude: true` to all layers.
- **Result**: The map visualization regressed significantly (missing arcs, "old" look). Likely due to `HeatmapLayer` configuration errors crashing the DeckGL overlay.
- **Fix**: Reverted `frontend/src/components/GlobalObservatory/MapLayerManager.tsx` and `docs/adrs/ADR-0004-map-projection-strategy.md` to the stable checkpoint (`b0fb9af`).

### Current Status
- **Restored**: Stable baseline with visible arcs, nodes, and new layout.
- **Pending**:
    - Proper Heatmap implementation (needs careful testing).
    - Fix for Pacific crossing (clipping) - try `wrapLongitude` on Arcs *only* first.
